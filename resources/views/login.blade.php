...
@section('content')
    ...
    @endif --}}
            ...
            @if (session()->has('success'))
                <div class="bg-blue-500 text-white text-center italic p-2 rounded text-sm mb-2"
                    style="background-color: green">
                    {{ session('success') }}</div>
            @endif

            @if (session()->has('error'))
                <div class="bg-red-600 text-white text-center italic p-2 rounded text-sm mb-2" style="background-color: red">
                    {{ session('error') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('login.store') }}" class="space-y-5">
                ...
                <div class="mb-2">
                    ...
                    <input id="email" name="email" type="email" autofocus value="{{ old('email') }}"
                        class="w-full border rounded-lg p-3 focus:ring-2 focus:ring-indigo-500" placeholder="seu@email.com">
                    @error('email')
                ...
                <div class="mb-2">
                    ...
                    <input id="password" name="password" type="password"
                        class="w-full border rounded-lg p-3 focus:ring-2 focus:ring-indigo-500" placeholder="••••••••">
                    @error('password')
                        <span class="text-red-600 italic text-center">{{ $message }}</span>
                    @enderror
                </div>
